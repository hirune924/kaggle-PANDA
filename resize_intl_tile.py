...
def load_img(img_name, K=16):
    WINDOW_SIZE = 128
    STRIDE = 128
    #K = 16
    image, best_coordinates, best_regions, win = generate_patches(img_name, window_size=WINDOW_SIZE, stride=STRIDE, k=K, auto_ws=True)
    WINDOW_SIZE = win
    STRIDE = WINDOW_SIZE
    #glued_image = glue_to_one_picture(best_regions, window_size=WINDOW_SIZE, k=K)
    glued_image = glue_to_one_picture_from_coord(img_name, best_coordinates, window_size=WINDOW_SIZE, k=K)
    return glued_image
